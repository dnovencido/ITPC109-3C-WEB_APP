<?php
    ...
    function validate_form_dish($name, $price, $thumbnail, $thumbnail_tmp_name) {
        ...
        if(!$thumbnail && !$thumbnail_tmp_name) {
            $validation_errors[] = "The thumbnail of the dish is required.";
        }
    ...
    function get_all_dishes($filter = [], $pagination = []) {
        ...
        // The base query
        $query = "SELECT `d`.`id`, `d`.`name` as `dish`, `d`.`price`, `d`.`thumbnail`, `u`.`name` as `created_by`, `d`.`created_at`, `d`.`updated_at` 
                FROM `dishes` as `d` 
                INNER JOIN `users` as `u` ON `u`.`id` = `d`.`user_id`";
        ...
        if(!empty($filter)) {
            ...
            // Iterate over each filter to build the conditions
            ...
            $where_clause = [];  // Array to hold combined conditions
        ...
        // Execute the count query and store the total number of records
        ...
        // Append order by clause to the main query for sorting
        ...
        return $dishes; // Return the array of dishes including the total count and the result set
    }  
